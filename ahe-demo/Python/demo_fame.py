...
# https
keyManagementAddress = "https://localhost"
keyManagementPort = 6902

scriptDir = os.path.dirname(__file__)
soDir = os.path.join(scriptDir, "../../ahe-library/prebuild/linux-x86-64/libahe.so")
if os.environ.get("SIM", None) == "RPI":
    soDir = os.path.join(scriptDir, "../../ahe-library/prebuild/linux-armv7/libahe.so")
...
def main():
    ...
    savePubToWallet(a, keyManagementAddress, keyManagementPort, wallets["pub_keys"], ca)
    # get private keys from the key authorities
    saveSecToWallet(a, keyManagementAddress, keyManagementPort, wallets["sec_keys_" + uuid], uuid, attribs, ca)
    ...
    proof = postVerificationKey(keyManagementAddress, keyManagementPort, uuid, verKey, ca)

    # encrypt and sign data (return is a json string)
    enc = encryptAndSign(a, ["message1", "message2"], ["machine123_for_fame", "admin AND machine123_for_fame"],
    ...
    enc2 = a.Encrypt("message3", "machine123_for_fame", pk)
